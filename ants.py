...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        current = self.place
        distance = 0
        while current.is_hive == False:
            if self.lower_bound <= distance <= self.upper_bound:
                if random_bee(current.bees) != None:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            reflective = amount + self.damage
        for bees in copy:
            bees.reduce_health(reflective)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        lastplace = self.place.exit
        
        while lastplace != None:
            if lastplace.ant != None:
                print("Debug ", "Doubling ", lastplace.ant.name)
                lastplace.ant.double()
                if lastplace.ant.is_container == True and lastplace.ant.ant_contained != None:
                    lastplace.ant.ant_contained.double()
            lastplace = lastplace.exit
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        print(f"Debug: {target.name}")
        super().throw_at(target)
        if target is not None:
            target.SlowTurns = 5
            def target_action(gamestate):
                print(f"Debug: GameState Time {target.name}")
                if target.SlowTurns <= 0 or is_Even(gamestate.time) == True:
                    Bee.action(target, gamestate)
                target.SlowTurns = target.SlowTurns - 1
